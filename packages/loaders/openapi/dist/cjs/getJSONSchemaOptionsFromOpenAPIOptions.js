...
async function getJSONSchemaOptionsFromOpenAPIOptions(name, { source, fallbackFormat, cwd, fetch: fetchFn, endpoint, schemaHeaders, operationHeaders, queryParams = {}, selectQueryOrMutationField = [], logger = new utils_1.DefaultLogger('getJSONSchemaOptionsFromOpenAPIOptions'), jsonApi, HATEOAS, }, hateoasContext) {
    ...
    if (typeof source === 'string') {
        source = string_interpolation_1.stringInterpolator.parse(source, {
        ...
        for (const method in pathObj) {
            ...
            for (const responseKey in methodObj.responses) {
                ...
                if (hateOasConfig &&
                    ...
                    await Promise.all(Object.keys(schemaObj.properties[hateOasConfig.linkObjectIdentifier].properties).map(async (linkName) => {
                        ...
                        if (xLinkObj) {
                            ...
                            function findActualOperationAndPath(possibleName, possibleOasDoc, possibleMethodObjFieldMap) {
                                ...
                                if (actualOperation) {
                                    const args = {};
                                    const paramsInLink = (0, string_interpolation_1.getInterpolationKeys)(xLinkHref);
                                    const paramsInTarget = (0, string_interpolation_1.getInterpolationKeys)(actualPath);
                                    ...
                                    if (possibleName === name) {
                                        links[linkName] = {
                                            get fieldName() {
                                                const linkOperationConfig = possibleMethodObjFieldMap.get(actualOperation);
                                        ...
                                        if (succesfulRes && 'content' in succesfulRes) {
                                            ...
                                            if ('$ref' in contentSchema) {
                                                ...
                                                if (contentSchema.type === 'array') {
                                                    const items = (0, utils_2.asArray)(contentSchema.items);
                                            ...
                                            if (objectSchema) {
                                                ...
                                                fusion_composition_1.futureAdditions.push({
